...
skipIfLowerThanDjango17 = skipIf(
    django.VERSION < (1, 7),
...
class TestMigrations(TransactionTestCase):
    ...
    def test_stable_sorting_after_migration(self):
        ...
        self.assertEqual(list(gallery.photos.values_list(
            'name', flat=True)), ['A', 'C', 'B'])
        self.assertEqual(list(gallery.photos2.values_list(
            'name', flat=True)), ['B', 'A', 'C'])

        m2m_set(gallery, "photos", [p3, p2])
        self.assertEqual(
            list(gallery.photos.values_list('name', flat=True)), ['B', 'C'])
