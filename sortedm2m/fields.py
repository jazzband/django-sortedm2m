    ...
    class SortedRelatedManager(RelatedManager):
        def _apply_rel_ordering(self, queryset):
            return queryset.extra(
                order_by=[
                    "%s.%s" % (self.through._meta.db_table, self.through._sort_field_name)
                ]
            )
...
class SortedManyToManyField(_ManyToManyField):
    ...
    def __init__(self, to, sorted=True, base_class=None, **kwargs):
        ...
        self.sort_value_field_name = kwargs.pop(
            "sort_value_field_name", SORT_VALUE_FIELD_NAME
        )
    ...
    def check(self, **kwargs):
        return super(SortedManyToManyField, self).check(
            **kwargs
    ...
    def contribute_to_class(self, cls, name, **kwargs):
        if not self.sorted:
            return super(SortedManyToManyField, self).contribute_to_class(
                cls, name, **kwargs
            )
        ...
        if rel.symmetrical and (
            rel.model == "self" or rel.model == cls._meta.object_name
        ):
    ...
    def create_intermediate_model(self, klass):
        base_classes = (
            (self.base_class, models.Model) if self.base_class else (models.Model,)
        )

        return create_sortable_many_to_many_intermediary_model(
            self, klass, self.sort_value_field_name, base_classes=base_classes
        )
